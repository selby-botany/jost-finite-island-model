...
def test_engine_backend_selector_stays_enabled_once_a_batch_is_configured(
    ...
    settled = drive(
        ...
        ready=_INPUT_SCREEN_READY,
        trigger=(
            "document.getElementById('field-engine_backend').value = 'auto';"
            "document.getElementById('field-engine_backend')"
...
def test_run_simulation_submits_the_chosen_engine_backend_for_a_batch(
    ...
    settled = drive(
        ...
        trigger=(
            ...
            "n.value = '20';"
            "n.dispatchEvent(new Event('input', {bubbles: true}));"
            "window.__submittedEngineBackend = null;"
            "window.pywebview.api.start_run = function (values) {"
            "  window.__submittedEngineBackend = values.engine_backend;"
            "  return Promise.resolve({ok: true});"
            "};"
            "document.getElementById('run-button').click();"
        ),
        read="window.__submittedEngineBackend",
        is_ready=lambda value: value is not None,
    )
