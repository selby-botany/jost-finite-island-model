...
def test_rerun_study_new_seed_mode_produces_a_different_seed(
    ...
    assert result == {"ok": True, "completed": 1, "failed": 0}
    directories = api.list_studies()[0]["runDirectories"]
    assert len(directories) == 2
    new_directory = next(d for d in directories if d != str(output))
    new_manifest = read_manifest(Path(new_directory) / "manifest.json")
    assert new_manifest.parameters["seed"] != 1
...
def test_rerun_study_same_seed_mode_reuses_the_original_seed(
    ...
    new_directory = next(d for d in directories if d != str(output))
    new_manifest = read_manifest(Path(new_directory) / "manifest.json")
    assert new_manifest.parameters["seed"] == 42
...
def test_rerun_study_reports_an_unknown_study(
    ...
    result = Api().rerun_study("study-ffffffff")
...
def test_rerun_study_reports_an_empty_study(
    ...
    assert result["ok"] is False
    assert "no runs to re-run" in result["message"]
...
def test_get_set_rerun_seed_mode_round_trips(tmp_path: Path) -> None:
    ...
    preferences_path = tmp_path / "preferences.json"
    api = Api(preferences_path=preferences_path)
    assert api.get_rerun_seed_mode() == "new"

    result = api.set_rerun_seed_mode("same")

    assert result == {"ok": True, "value": "same"}
    assert Api(preferences_path=preferences_path).get_rerun_seed_mode() == "same"
...
def test_set_rerun_seed_mode_rejects_an_unknown_value(tmp_path: Path) -> None:
    ...
    result = api.set_rerun_seed_mode("sometimes")
