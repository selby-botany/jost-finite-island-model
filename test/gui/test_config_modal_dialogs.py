...
always-visible `screen-configure`); `modal-presets` and `modal-about`
are what remain (`modal-save-preset` uses a real `<form method="dialog">`
with its own Accept/Cancel buttons instead, design §4.7 — neither is
`[data-modal-close]`). `index.html`'s own comment above `modal-presets`
records why its own "Cancel" button still needs an explicit
`tabindex="0"` (a dialog-fix report's own (b): this project's WKWebView
host only includes an element in the `Tab` order that the author
explicitly opted in with `tabindex`, unless the OS-level "Full Keyboard
Access" setting is on — without it, "Cancel" is reachable by pointer but
not by keyboard, exactly the gap `Tab`, `Tab`, ..., `Return` is supposed
to close). This test answers the same question `test_webui_global_scope.py`
...
def test_every_modal_close_button_has_an_explicit_tabindex() -> None:
    ...
    assert len(buttons) == 3, (
        f"expected exactly 3 [data-modal-close] buttons, found {len(buttons)}"
    )
