...
| …`migrate`/`mutate`/`drift` themselves operated on dense arrays instead of one Python loop per deme, for the bounded-K (finite-alleles) mutation model? | `fim()`'s own `engine_backend="generational-vector"` (library only, no CLI flag yet), scoped to `mutation_model="finite_alleles"` and `migrant_sampling="continuous"` — a config outside that scope raises `ValueError` naming the violated constraint | a third engine implementation, `VectorizedAdvancer`, converts each replicate's own state to a dense `(deme, allele)` array once per generation and runs `migrate`/`mutate`/`drift` fused on that array (`fim.model.vectorized`), statistically — not bit-identically — equivalent to the other two backends. Needs the optional `numba` dependency unconditionally (no separate `jit` toggle). Measured against `"lineal"` at this project's own reference scale: close to parity (~0.82x wall-clock), not yet a clean win — the per-generation round-trip back to `ModelState` is a real, secondary cost this backend does not yet eliminate |
