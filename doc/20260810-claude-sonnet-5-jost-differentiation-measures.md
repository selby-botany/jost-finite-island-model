...
know what $F_{ST}$ is and have opinions about it. Most of the extra
...
error can land, so $\mu \approx \mu_b \cdot L$ for a per-base-pair rate
$\mu_b$ and length $L$ — which is the whole content of Eq. 5. The
consequence is sharp, and it is one more instance of the dissociation this
paper is built around: **locus length is invisible to $G_{ST}$ and
decisive for $D$.** Length acts only through $\mu$, and $\mu$ cancels out
of the equilibrium $G_{ST}$ (Eq. 2) whenever $\mu \ll m$, while
...
else entirely: the right to **combine** loci — averaging $G_{ST}$
across many markers to estimate $Nm$, taking harmonic means of $D$ and
$\mu$, and attaching confidence intervals (all in Part VI). For that
...
of the paper's arguments: $F_{ST}$ was *originally* defined in terms of
pedigrees, and $G_{ST}$ — its allele-frequency stand-in — is therefore
...
**Mutation** introduces brand-new alleles at rate $\mu$ per locus per
...
| Marker type | Rough mutation rate $\mu$ per generation |
|---|---|
| Single nucleotide site | $10^{-9}$ – $10^{-8}$ |
| Typical protein-coding gene | $10^{-6}$ – $10^{-5}$ |
| Microsatellite | $10^{-4}$ – $10^{-3}$ |
...
- **Microsatellite**: a short repeated motif whose repeat count varies,
  e.g. `CACACACA…`. Many alleles, very high $\mu$. Historically the
...
VI: allelic differentiation depends strongly on $\mu$, so a marker with a
...
Given one deme's frequency vector $p = (p_1, p_2, \ldots, p_k)$, the
...
deme's pool. $\sum_i p_i^2$ is the probability they are the **same**
...
- $H < 1$ always, and $H \to 1$ only as the number of alleles
  $\to \infty$.
...
$J = n \cdot (1/n)^2 = 1/n$, so ${}^{H}D = n$ exactly. The
...
| $1$ | Shannon diversity, $\exp(\text{entropy})$ | Alleles weighted by their frequency | Proportional weight |
...
$(\sum_i p_i^2)^{1/(1-2)} = (\sum_i p_i^2)^{-1} = 1/J$. This ladder
...
- $H_S$ — the mean **within-deme** heterozygosity: compute $H$
  separately for each deme, then average. "How much variation does a
  typical deme hold internally?"
- $H_T$ — the **total** heterozygosity: pool all demes into one combined
...
not a count-weighted mean. (For $E_{ST}$, size weighting is available
and is in fact one of its selling points; see below.)

Necessarily $H_T \geq H_S$: pooling different demes can only add
variation. The gap $H_T - H_S$ is the raw material both families of
...
$F_{ST}$ (Wright, 1943, 1965) was originally defined in pedigree terms:
...
$G_{ST}$ (Nei, 1973) is the multi-allele version expressed purely in
allele frequencies:
...
$\theta$ (Weir & Cockerham, 1984) is an unbiased *estimator* of
$F_{ST}$ that corrects for finite sample size.

Because $G_{ST}$ is a function only of allele frequencies, the paper
notes it is **not measuring exactly the same thing as Wright's original**
$F_{ST}$ — it is a frequency-based stand-in, used throughout the paper
for analytical simplicity, on the premise that $F_{ST}$ and $\theta$
...
$G_{ST}$ is **undefined** when all demes are fixed for the same allele,
since then $H_T = 0$ and the formula divides by zero — an awkwardness
...
distance (Nei, 1972). Denominator: $1 - H_S$, i.e. $J_{\mathrm{within}}$.
...
**Entropy differentiation** $E_{ST}$, the $q = 1$ member:

```math
E_{ST} = \frac{E_T - E_S}{E_w}
```

$E_T$ is the Shannon entropy of the pooled demes, $E_S$ the mean
within-deme entropy weighted by deme size, and $E_w$ the entropy of the
*relative deme sizes* — $E_w = \ln d$ for **d** equally sized demes.

Two advantages the paper claims for $E_{ST}$:
...
2. It has **stronger monotonicity**: adding a new private allele to a
   deme *always* increases $E_{ST}$. That is not true of **D** — because
   **D** weights alleles by squared frequency, adding a rare private
   ...
   wart on **D** and a reason to prefer $E_{ST}$ in conservation
   settings, where discovering a new unique allele should never make a
   differentiation index fall.

**Allele-number differentiation** $K_{ST}$, the $q = 0$ member:

```math
K_{ST} = 1 - \frac{K_T/K_S - d}{1 - d}
```

$K_T$ is the total number of distinct alleles across all demes; $K_S$
is the unweighted mean number of alleles per deme. Frequencies are ignored
entirely — only presence/absence counts. $K_{ST}$ has the **strongest**
...
${}^{q}D_T$ and ${}^{q}D_S$ are the total and mean-within-deme Hill
numbers of order **q**. Setting the dial: $q = 2$ gives Jost's **D**;
$q = 1$, taken as a limit, gives $E_{ST}$; $q = 0$ gives $K_{ST}$.

Two notes on that formula worth keeping:

- For $q \neq 0, 1$, demes **must** be given equal statistical weight if
  the goal is to compare *relative* allele frequencies between demes.
- ${}^{q}D_T / {}^{q}D_S$ is itself the **between-group diversity**
...
| | Fixation family ($G_{ST}$) | Allelic differentiation family ($D$) |
|---|---|---|
| Formula | $(H_T - H_S)/H_T$ | $[(H_T-H_S)/(1-H_S)]\cdot d/(d-1)$ |
| Denominator | $H_T$ | $1 - H_S$ |
...
| Depends on $\mu$? | Barely, when $\mu \ll m$ | Strongly, always |
| Equilibrium driver | $Nm$ (absolute migrant count) | $m/(\mu(d-1))$ |
| Comparable across loci? | Yes (that is its design goal) | Only among loci with similar $\mu$ |
| Comparable across species? | Not reliably ($H_T$ varies) | Yes |
...
ten demes are fixed**, so $H = 0$ in each one and $H_S = 0$
throughout — which makes $G_{ST} = (H_T - 0)/H_T = 1$ identically, no
...
| Scenario | $H_S$ | $H_T$ | $G_{ST}$ | $D$ |
|---|---|---|---|---|
| 9 + 1 | 0.00 | 0.18 | **1.00** | **0.20** |
| 5 + 5 | 0.00 | 0.50 | **1.00** | **0.5556** |
| All different | 0.00 | 0.90 | **1.00** | **1.00** |

$G_{ST}$ is 1.00 in all three cases and cannot distinguish them — which
is correct, since all three are fully fixed. But the conservation
implications differ completely. A manager acting on $G_{ST}$ alone would
...
intuitive reading. Write out all $10\cdot9/2 = 45$ pairs of demes; for
...
all alleles equally frequent within a deme. Here $G_{ST}$ is near
...
| Species | Shared alleles | $H_S$ | $H_T$ | $G_{ST}$ | **D** |
|---|---|---|---|---|---|
...
at all and yet has a *lower* $G_{ST}$ than species B, which shares half
of them.** A manager ranking by $G_{ST}$ would rank species B as the
...
alleles, almost none shared. The paper reports $H_T = 0.97$,
$H_S = 0.95$, with effective numbers 38.8 and 20.4. Back-solving from
the effective numbers gives the more precise $H_T = 0.9742$,
$H_S = 0.9510$; the calculation below uses those.
...
|---|---|---|
| $G_{ST}$ | 0.024 | Demes are far from fixation — **true, and irrelevant** |
| "98% within-group" | — | An artifact of **H**'s ceiling — **meaningless** |
| ${}^{H}D_T / {}^{H}D_S$ | 1.90 of a max 2.00 | Demes are nearly disjoint |
| **D** | **0.95** | Demes are 95% allelically differentiated |
| $E_{ST}$ | 0.90 | Same conclusion, frequency-weighted |
| $K_{ST}$ | 0.77 | 77% of each deme's alleles are unique to it |

Both demes must be protected — the $G_{ST}$-driven analysis says the
opposite. The paper also flags that the ordering $D (0.95) > E_{ST}
(0.90) > K_{ST} (0.77)$ is itself informative: **D** weights by squared
frequency (dominated by the most common alleles), $E_{ST}$ weights by
plain frequency (the average allele), and $K_{ST}$ ignores frequency
...
| $K_{ST}$ | **0.00** | Every allele is present in both demes; none is unique |
...
odd deme exists purely to keep $G_{ST}$ from dividing by zero). The
locus is neutral with a high mutation rate, $\mu = 0.001$ per
...
$G_{ST}$ **falls monotonically from unity to near zero over exactly the
...
ended diverse. (Hedrick's $G\'_{ST}$, a well-known rescaling of $G_{ST}$
...
This makes nonsense of the common habit of reading $G_{ST}$ against a
...
with $G_{ST}$ — it is what falls out of doing the partition correctly
...
alleles at rate $\mu$ under the infinite-alleles assumption (every
...
itself drifts, which is what makes the classical closed-form $F_{ST}$
...
When $m \gg \mu$ — the usual case for slowly mutating loci — the
$4N\mu$ term is negligible and this collapses to a function of **Nm**
and **d** alone:
...
Two consequences follow. First, $G_{ST}$ depends on the **absolute
...
give the same expected $G_{ST}$. Second, because $\mu$ has dropped
out, the expected $G_{ST}$ is **the same for all loci** with
$\mu \ll m$, so estimates from many loci can legitimately be averaged
...
The controlling quantity is $m/(\mu(d-1))$ — a ratio of migration rate to
...
diverge completely; with $m \gg \mu(d-1)$, $D \to 0$ and migration
homogenizes them.

### Why this kills the standard inference

| | Controlled by | Contains $N$? | Contains $\mu$? |
|---|---|---|---|
| $G_{ST}$ (nearness to fixation) | $Nm$ | yes | barely, when $\mu \ll m$ |
| $D$ (allelic differentiation) | $m/(\mu(d-1))$ | **no** | **strongly** |
...
one in which **N** plays no part and $\mu$ plays a decisive one. This
...
Since **D**'s equilibrium depends strongly on $\mu$, and $\mu$ spans
...
$\mu$; fast-mutating markers remain valid for *ranking* deme pairs, just
...
- **A weighting choice** (Part III): given real data from demes of
  ...
  size? **D** weights every deme equally by construction; $E_{ST}$
...
than large ones, and $H_S$ becomes an average over demes with genuinely
...
| $N$ used | $4Nm$ | Eq. 2 $G_{ST}$ |
|---|---|---|
| 1090 (arithmetic mean) | 43.6 | 0.018 |
| 111 (harmonic mean) | 4.44 | **0.153** |

Plugging in the *average* deme size understates $G_{ST}$ by roughly
...
$J_{between}/J_{within}$ on which $D$ is built. For the numbers above,
Eq. 4 returns $D \approx 0.083$ whichever deme size you use, because it
...
$\mu$; unequal sizes in real metapopulations usually come with unequal
migration rates, and then $m$ is not a scalar and the derivation's
premise is gone. The $\approx$ is doing real work.
...
part of the argument. **Neither $G_{ST}$ nor $D$ should be used to
...
completely isolated can still show a low $G_{ST}$ simply because
...
$E_{ST}$, and $K_{ST}$ at the loci of actual interest, rather than
...
$G_{ST} = 1.00$ while $D = 0.20$ — numerically identical to the first
worked example above. Split five-five and $G_{ST}$ is still $1.00$
...
With only two alleles, $G_{ST}$'s dependence on within-group
heterozygosity stops mattering much:

| Configuration | $G_{ST}$ | $D$ |
|---|---|---|
...
1. **Report both families, and label them by what they measure.**
   Interpret $G_{ST}$, $F_{ST}$, and $\theta$ as **nearness to
   fixation** (as Wright himself did), and $D$, $E_{ST}$, $K_{ST}$
   as **allelic differentiation**. They are complementary, not competing.
2. **$D$ is not an estimator of $F_{ST}$ or $G_{ST}$.** Treating it
   as a "corrected $G_{ST}$" is a category error the paper names
   explicitly.
...
4. **Report the profile across $q$, not one index.** $D$ ($q=2$),
   $E_{ST}$ ($q=1$), and $K_{ST}$ ($q=0$) weight allele
   ...
   estimate reliably from small samples; $E_{ST}$ has the most robust
   monotonicity and partitioning properties, handles unequal deme sizes,
   and suits hierarchical analysis; $K_{ST}$ has the strongest
...
6. **Interpret magnitudes and confidence intervals, not p-values.**
   ...
   question; effect size is. Relatedly, $H_T$ varies across pairwise
   comparisons, so pairwise $G_{ST}$ values are not truly comparable to
...
|---|---|---|
| "$F_{ST}$/$G_{ST}$ measures how different populations are." | **False.** It measures nearness to fixation. | Part III |
...
| "Hedrick's $G'_{ST}$ fixes $G_{ST}$." | **Partial at best.** Addresses only the high-heterozygosity end, not the low. | Part IV |
| "$G_{ST}$ and $D$ can be used to estimate current migration." | **False.** Both integrate historical demography. | Part VI |
...
$K_{ST} \ll D$ says the groups share their rare categories but differ
in their typical ones; $K_{ST} \gg D$ says the reverse. Neither fact is
...
**The relation to Jaccard and Sørensen.** $K_{ST}$ is the one-complement
...
that generative process, there is no $\mu$, $m$, $N$, or
equilibrium, and none of those formulas means anything.

**$G_{ST}$ and the whole fixation family.** "Nearness to fixation" is a
...
$F_{ST}$ interpretive scales should not be imported.
...
$E_{ST}$ can weight by group size. Outside genetics the analogous choice
